...
try:
    ...
    def compat_urllib_parse_unquote(string, encoding="utf-8", errors="replace"):
        ...
        for i in range(1, len(bits), 2):
            append(
                compat_urllib_parse_unquote_to_bytes(bits[i]).decode(encoding, errors)
            )
            append(bits[i + 1])
        return "".join(res)
...
try:
    args = shlex.split("中文")
    assert (
        isinstance(args, list) and isinstance(args[0], compat_str) and args[0] == "中文"
    )
...
if sys.version_info >= (3, 0):
    ...
    def compat_setenv(key, value, env=os.environ):
        def encode(v):
            ...
            return (
                v.encode(get_filesystem_encoding()) if isinstance(v, compat_str) else v
            )
...
def workaround_optparse_bug9161():
    ...
    try:
        ...
        def _compat_add_option(self, *args, **kwargs):
            enc = lambda v: (
                v.encode("ascii", "replace") if isinstance(v, compat_str) else v
            )
...
try:
    ...
    if platform.python_implementation() == "IronPython" and sys.version_info < (
        2,
        7,
        8,
    ):
