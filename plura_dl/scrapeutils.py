...
class Logger(object):
    ...
    def __init__(self, logpath):
        ...
        with open(self.logpath, "wt") as f:
            f.close

    def debug(self, msg):
        msg = datetime.today().strftime("%Y-%m-%d %H:%M:%S") + " | " + msg
        print(msg)
        with open(self.logpath, "at") as f:
            f.write(msg + "\n")
...
def store_dict_as_json(dictionary, filepath):
    ...
    with codecs.open(filepath, "w", "utf-8") as f:
        json_string = json.dumps(
            dictionary, sort_keys=True, indent=4, ensure_ascii=False
        )
        f.write(json_string)
...
def enter_hibernation():
    while True:
        ...
        if userinput in ["y", "Y", "yes", "YES"]:
            ...
            userinput = input(
                "Not entering valid option again will terminate. This is you last chance [y/N] :"
            )
            if userinput in ["y", "Y", "yes", "YES"]:
                continue
            else:
