...
class FFmpegPostProcessor(PostProcessor):
    ...
    def _determine_executables(self):
        ...
        if self._downloader:
            ...
            if location is not None:
                ...
                self._paths = dict((p, os.path.join(location, p)) for p in programs)
                self._versions = dict(
                    (p, get_ffmpeg_version(self._paths[p])) for p in programs
                )
        if self._versions is None:
            self._versions = dict((p, get_ffmpeg_version(p)) for p in programs)
    ...
    def get_audio_codec(self, path):
        ...
        try:
            ...
            return None
        output = (stdout_data if self.probe_available else stderr_data).decode(
            "ascii", "ignore"
        )
        if self.probe_available:
            audio_codec = None
...
class FFmpegVideoConvertorPP(FFmpegPostProcessor):
    ...
    def run(self, information):
        ...
        if information["ext"] == self._preferedformat:
            self._downloader.to_screen(
                "[ffmpeg] Not converting video file %s - already is in target format %s"
                % (path, self._preferedformat)
            )
            return [], information
        options = []
        ...
        self._downloader.to_screen(
            "["
            + "ffmpeg"
            + "] Converting video from %s to %s, Destination: "
        ...
        for lang, sub_info in subtitles.items():
            ...
            if ext != "webm" or ext == "webm" and sub_ext == "vtt":
                ...
                if not webm_vtt_warn and ext == "webm" and sub_ext != "vtt":
                    webm_vtt_warn = True
                    self._downloader.to_screen(
                        "[ffmpeg] Only WebVTT subtitles can be embedded in webm files"
                    )
...
class FFmpegFixupM3u8PP(FFmpegPostProcessor):
    def run(self, info):
        ...
        if self.get_audio_codec(filename) == "aac":
            ...
            self._downloader.to_screen(
                '[ffmpeg] Fixing malformed AAC bitstream in "%s"' % filename
            )
            self.run_ffmpeg(filename, temp_filename, options)
