    ...
    def _readUserConf():
        ...
        if xdg_config_home:
            ...
            userConfFile = os.path.join(
                compat_expanduser("~"), ".config", "plura-dl", "config"
            )
            if not os.path.isfile(userConfFile):
                userConfFile = os.path.join(
                    compat_expanduser("~"), ".config", "plura-dl.conf"
                )
        ...
        if userConf is None:
            ...
            if appdata_dir:
                userConf = _readOptions(
                    os.path.join(appdata_dir, "plura-dl", "config"), default=None
                )
                if userConf is None:
                    userConf = _readOptions(
        ...
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "plura-dl.conf"), default=None
            )
        if userConf is None:
            userConf = _readOptions(
                os.path.join(compat_expanduser("~"), "plura-dl.conf.txt"), default=None
            )
