  ...
  rpc {
    ...
    def _download_course_rpc(self, course_id, url, display_id):
        response = self._download_json(
            ...
            display_id,
            data=json.dumps(
                {"query": self._GRAPHQL_COURSE_TMPL % course_id, "variables": {}}
            ).encode("utf-8"),
        ...
        raise ExtractorError(
            "%s said: %s" % (self.IE_NAME, response["error"]["message"]), expected=True
        )
  ...
  viewClip(input: {
        ...
        if all(
            ...
            if any(p in response for p in (MUST_AGREE, ">Disagree<", ">Agree<")):
                raise ExtractorError(
                    ...
                    "log in and agree with what Pluralsight requires." % MUST_AGREE,
                    expected=True,
                )
    ...
    def _get_subtitles(self, author, clip_idx, clip_id, lang, name, duration, video_id):
        ...
        if not captions:
            ...
            captions = self._download_json(
                "%s/player/retrieve-captions" % self._API_BASE,
                video_id,
                "Downloading captions JSON",
                "Unable to download captions JSON",
                fatal=False,
        ...
        for num, current in enumerate(subs):
            ...
            srt += os.linesep.join(
                (
                    "%d" % num,
                    "%s --> %s"
                    % (srt_subtitles_timecode(start), srt_subtitles_timecode(end)),
                    text,
                    os.linesep,
                )
            )
        ...
        for ext, qualities_ in allowed_qualities:
            for quality in qualities_:
                ...
                try:
                    ...
                    viewclip = self._download_json(
                        "%s/video/clips/viewclip" % self._API_BASE,
                        display_id,
                        "Downloading %s viewclip JSON" % format_id,
                        fatal=False,
