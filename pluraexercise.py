...
from plura_dl.dependentutils import TimeoutException, set_chrome_driver, wait_for_access

from selenium.webdriver.chrome.options import Options
from pluradl import set_directory
...
ALT_DOWNLOAD_EXERCISE_FILE = (
    r"/html/body/div[1]/div[3]/div/div[2]/section/div[4]/div/div/button"
)
...
def download_routine(driver, course, sleep_time=10):
    ...
    try:
        ...
        try:
            course_text = driver.find_element_by_class_name(
                "l-course-page__content"
            ).text
        except:
            course_text = ""
        ...
        if materials_check:
            try:
                wait_for_access(
                    driver, ALT_DOWNLOAD_EXERCISE_FILE, timer=sleep_time
                ).click()

            except TimeoutException:
                print(
                    course,
                    "did not succeeded. The course might not be in your subscription or it`s not available anymore. Tagging it ...",
                )
