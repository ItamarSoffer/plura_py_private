...
class TestPluraDLCookieJar(unittest.TestCase):
    def test_keep_session_cookies(self):
        ...
        try:
            ...
            self.assertTrue(
                re.search(
                    r"www\.foobar\.foobar\s+FALSE\s+/\s+TRUE\s+0\s+PluraDLExpiresEmpty\s+PluraDLExpiresEmptyValue",
                    temp,
                )
            )
            self.assertTrue(
                re.search(
                    r"www\.foobar\.foobar\s+FALSE\s+/\s+TRUE\s+0\s+PluraDLExpires0\s+PluraDLExpires0Value",
                    temp,
                )
            )
        finally:
    ...
    def test_strip_httponly_prefix(self):
        ...
        def assert_cookie_has_value(key):
            self.assertEqual(
                cookiejar._cookies["www.foobar.foobar"]["/"][key].value, key + "_VALUE"
            )
