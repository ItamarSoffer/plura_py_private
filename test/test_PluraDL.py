...
class TestFormatSelection(unittest.TestCase):
    ...
    def test_default_format_spec(self):
        ...
        pdl = PDL({})
        self.assertEqual(
            pdl._default_format_spec({"is_live": True}), "best/bestvideo+bestaudio"
        )

        pdl = PDL({"simulate": True})
        self.assertEqual(
            pdl._default_format_spec({"is_live": True}), "bestvideo+bestaudio/best"
        )
        ...
        pdl = PDL({})
        self.assertEqual(
            pdl._default_format_spec({}, download=False), "bestvideo+bestaudio/best"
        )
        self.assertEqual(
            pdl._default_format_spec({"is_live": True}), "best/bestvideo+bestaudio"
        )
...
class TestPluraDL(unittest.TestCase):
    ...
    def test_format_note(self):
        ...
        assertRegexpMatches(self, pdl._format_note({"vbr": 10,}), r"^\s*10k$")
        assertRegexpMatches(self, pdl._format_note({"fps": 30,}), r"^30fps$")
    ...
    def test_playlist_items_selection(self):
        entries = [
            {"id": compat_str(i), "title": compat_str(i), "url": TEST_URL,}
            for i in range(1, 5)
        ]
        playlist = {
            "_type": "playlist",
    ...
    def test_urlopen_no_file_protocol(self):
        ...
        pdl = PDL()
        self.assertRaises(
            compat_urllib_error.URLError, pdl.urlopen, "file:///etc/passwd"
        )
