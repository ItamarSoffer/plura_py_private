...
class BuildHTTPServer(
    compat_socketserver.ThreadingMixIn, compat_http_server.HTTPServer
):
...
class SERVICE_TABLE_ENTRY(ctypes.Structure):
    _fields_ = [("lpServiceName", LPTSTR), ("lpServiceProc", START_CALLBACK)]
...
def win_OpenSCManager():
    ...
    if not res:
        raise Exception(
            "Opening service manager failed - " "are you running this as administrator?"
        )
    return res
...
def win_uninstall_service(service_name):
    ...
    try:
        ...
        if not h:
            raise OSError(
                "Could not find service %s: %s" % (service_name, ctypes.FormatError())
            )
...
def win_service_report_event(service_name, msg, is_error=True):
    ...
    try:
        ...
        if not advapi32.ReportEventW(
            event_log, type_id, 0, event_id, None, len(lines), 0, lines, None
        ):
...
def win_service_main(service_name, real_main, argc, argv_raw):
    try:
        ...
        if not h:
            raise OSError("Handler registration failed: %s" % ctypes.FormatError())
...
def win_service_start(service_name, real_main):
    try:
        ...
        dispatch_table = _ctypes_array(
            SERVICE_TABLE_ENTRY,
            [
                SERVICE_TABLE_ENTRY(service_name, cb),
        ...
        for node in ("Wow6432Node\\", ""):
            try:
                key = compat_winreg.OpenKey(
                    ...
                    r"SOFTWARE\%sPython\PythonCore\%s\InstallPath"
                    % (node, python_version),
                )
                try:
...
class GITInfoBuilder(object):
    def __init__(self, **kwargs):
        ...
        path = os.path.join(
            os.environ["APPDATA"], "Build archive", self.repoName, self.user
        )
        ...
        try:
            subprocess.check_output(
                [
                    "git",
                    "clone",
                    "git://github.com/%s/%s.git" % (self.user, self.repoName),
                    self.buildPath,
                ]
            )
    ...
    def build(self):
        try:
            proc = subprocess.Popen(
                [os.path.join(self.pythonPath, "python.exe"), "setup.py", "py2exe"],
...
class DownloadBuilder(object):
    ...
    def build(self):
        ...
        self.handler.send_header(
            "Content-Disposition",
            "attachment; filename=%s" % os.path.split(self.srcPath)[-1],
        )
...
class Builder(
    PythonBuilder, GITBuilder, PluraDLBuilder, DownloadBuilder, CleanupTempDir, Null
):
    pass
    ...
    def do_GET(self):
        ...
        if path:
            ...
            if action in self.actionDict:
                try:
                    builder = self.actionDict[action](
                        path=path, handler=self, **paramDict
                    )
                    builder.start()
                    try:
                        builder.build()
